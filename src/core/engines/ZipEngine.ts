...
export class ZipEngine extends BaseEngine {
  readonly id = 'zip-engine';
  readonly name = 'Standard ZIP Engine';
  readonly description = 'High-compatibility PKZip archive generator with multi-threaded Deflate.';
  readonly supportedModes: CompressionMode[] = ['fast', 'balanced', 'maximum'];
  ...
  public async compress(
    ...
    // Map mode to Deflate compression level
    let level = 6;
    if (this.config.mode === 'fast') level = 1;
    if (this.config.mode === 'maximum') level = 9;
    ...
    const processSingleFile = async (item: FileItem): Promise<void> => {
      ...
      // Handle large files (e.g. 500MB to 15GB) by chunking if needed
      // For standard files < 64MB: single chunk
      ...
      if (!response.success || !response.compressedBuffer) {
        throw new Error(response.error || `Failed to compress ${item.relativePath}`);
      }
